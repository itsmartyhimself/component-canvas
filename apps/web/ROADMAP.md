...
- Demo-only: `StaleViewerTrigger` inside `app-shell.tsx` fires a warning-tone toast on a 30s `setTimeout`. Replace with a Supabase Realtime subscription on the instance channel watching `last_synced_commit_sha` (per architecture-brief §3 Stale viewer detection). Toast uses `duration: Infinity` deliberately — the staleness condition persists until refresh/dismiss.
- Connect-repo flow no longer fires a toast on submit; `router.push("/")` is the only feedback. If richer feedback is wanted later, prefer an inline syncing state on the new repo row (or route directly into the new instance's AppShell which renders syncing inline) over reintroducing a dashboard toast — the toast system is intentionally scoped to AppShell only.
...
- Toast uses `sonner.toast.custom()` with `unstyled: true` so the dark pill renders fully via the `<ToastBody>` component. If sonner's swipe-to-dismiss / stack behaviour misbehaves, swap to a custom Headless toast queue. `ToastProvider` mounts inside `AppShell` (not root layout) — calling `useToast` from dashboard code throws, which is the intended structural guarantee that toasts only fire from instance pages.
...
- Brand mark: the Mount wordmark renders via CSS mask from `/SVGs/mount-wordmark.svg`, tinted with `--color-text-primary` (auto light/dark). Used in the login HeroCard (height 20, `role="img"`) and dashboard nav (height 24, `aria-hidden`, inside the `aria-label="Mount home"` link). `/SVGs/mount-glyph.svg` is parked unused — wire it in if a sidebar/compact brand surface lands.
...
- Stale-viewer notification now ships as a `warning`-tone variant of the existing toast (per Pencil Section-Toast-v2 → Toast-Display → `Toast-banner`). Trigger lives in `app-shell.tsx`; banner component fully removed. Pencil's 13/500 "Refresh" action text maps to `.type-3 font-medium` (12/500) — same nearest-token match used for the success/error toasts' action labels. Toast `boxShadow` uses `var(--shadow-layered)` rather than a one-off literal; shadow tokens go to `none` in dark mode, which is the correct behavior for a light-pill-on-dark-canvas.
- DashboardNav sits flush (no border-bottom). A 24px-tall gradient fade sits as a sibling in the layout (`absolute; top: 60`) so scrolled main content dissolves under the nav rather than slamming a hard edge. Z-index 1, pointer-events: none.
- Round 3 corrections:
