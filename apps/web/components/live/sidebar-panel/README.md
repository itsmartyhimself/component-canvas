...
This is a demo shell. The registry (`lib/registry/data.ts`) is static and marked `// DEMO-ONLY`. Real repo-connect / server-backed registry is tracked in `apps/web/ROADMAP.md` under "Backend / Registry".
...
- Hosts `SidebarPanelProvider` for panel-specific ephemeral UI state (expansions, search query, collapsed state).
