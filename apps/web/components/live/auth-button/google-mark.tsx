interface GoogleMarkProps {
  size?: number
}

export function GoogleMark({ size = 18 }: GoogleMarkProps) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden
    >
      <path
        d="M22.56 12.25C22.56 11.47 22.49 10.72 22.36 10H12V14.26H17.92C17.66 15.63 16.88 16.79 15.71 17.57V20.34H19.28C21.36 18.42 22.56 15.6 22.56 12.25Z"
        fill="#4285F4"
      />
      <path
        d="M12.0002 23.0016C14.9702 23.0016 17.4602 22.0216 19.2802 20.3416L15.7102 17.5716C14.7302 18.2316 13.4802 18.6316 12.0002 18.6316C9.14018 18.6316 6.71018 16.7016 5.84018 14.1016H2.18018V16.9416C3.99018 20.5316 7.70018 23.0016 12.0002 23.0016Z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.0903C5.62 13.4303 5.49 12.7303 5.49 12.0003C5.49 11.2703 5.62 10.5703 5.84 9.91031V7.07031H2.18C1.43 8.55031 1 10.2203 1 12.0003C1 13.7803 1.43 15.4503 2.18 16.9303L5.03 14.7103L5.84 14.0903Z"
        fill="#FBBC05"
      />
      <path
        d="M12.0002 5.38C13.6202 5.38 15.0602 5.94 16.2102 7.02L19.3602 3.87C17.4502 2.09 14.9702 1 12.0002 1C7.70018 1 3.99018 3.47 2.18018 7.07L5.84018 9.91C6.71018 7.31 9.14018 5.38 12.0002 5.38Z"
        fill="#EA4335"
      />
    </svg>
  )
}
